...
function enviarMail($email,$subject, $message,$attach ,$isHTML ) {
    ...
    function pedidosProcesados($suc){
        ...
        /*$db->Query('SELECT count(*) AS PEDIDOS_ENVIADOS FROM nota_pedido p, nota_pedido_det d WHERE p.nro = d.nro_pedido AND  
                    p.__locald LIKE "'.$suc.'"   
                    AND   p.estado !="Abierta"   AND p.fecha BETWEEN "2013-09-11" and  CURRENT_DATE AND d.estado = "Enviado"');*/
        $db->Query("SELECT count(*) AS PEDIDOS_ENVIADOS FROM _audit_log_ where fecha = CURRENT_DATE AND x0_ = '$suc'");
