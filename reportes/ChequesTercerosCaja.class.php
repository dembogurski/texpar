...
class ChequesTercerosCaja {
    ...
    function main(){
        $t = new Y_Template("ChequesTercerosCaja.html");
        $t->Show("header");
        ...
        $my = new My();
        $sql = "SELECT nro_cheque,f_nro as factura, nombre AS banco,t.id_banco,cuenta,f_nro AS factura, DATE_FORMAT(fecha_ins,'%d-%m-%Y') AS fecha_ins,DATE_FORMAT(fecha_emis,'%d-%m-%Y') AS fecha_emis,
        ...
        FROM cheques_ter t, bancos b WHERE t.id_banco = b.id_banco AND suc like '$suc' AND  $campo_fecha BETWEEN '$fdesde' AND '$fhasta' and tipo like '$tipo_cheq' and m_cod like '$moneda' and ( nro_cheque like '%$nro_cheque%' or benef like '%$nro_cheque%' )    order by m_cod asc,fecha_ins,id_banco asc ";
        ...
        while($my->NextRecord()){
            ...
            if($recibido_adm == "Si"){
                $t->Set("chek_adm","<img src='../img/ok.png'>"); 
            }else{
                $t->Set("chek_adm",""); 
            }
             
            if($recibido_ger == "Si"){
                $t->Set("chek_ger","<img src='../img/ok.png'>"); 
